...
def main(day, create, force, submit, download, part, timeit_, verbose):
    ...
    print(
        f"Welcome to Advent of Code 2016 - {day=} - {part=} - {timeit_=} - "
        f"{verbose=} - {create=}"
    )
    ...
    if create:
        ...
        download_input_data(year="2016", day=day)
    else:
        # Run a specific day
        if download:
            download_input_data(year="2016", day=day)
...
def ensure_correct_directory():
    ...
    main_file = Path("src/adventofcode2016/main.py")
    if main_file.exists():
        return True
    else:
...
def create_solution(force, day):
    ...
    import_path = f"adventofcode2016.solutions.day{day}"
...
def run_day(data_path, day, day_module, part, submit):
    if part == "parta":
        ...
        if submit:
            submit_result(year=2016, day=day, part=part, result=result)
        return result
